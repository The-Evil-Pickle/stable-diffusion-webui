...
def apply_double(scale):
    def fun(p, x, xs):
        setattr(p, "cfg_scale", x)
        setattr(p, "steps", round(x * (2.8 + 0.4 * scale)) + 12 + round(scale * 40.0))
    return fun
...
axis_options = [
    ...
    AxisOption("CFG/Steps", float, apply_double(0.5), format_value_add_label),
    AxisOption("CFG/Steps, Low", float, apply_double(0.0), format_value_add_label),
    AxisOption("CFG/Steps, High", float, apply_double(1.0), format_value_add_label),
